function makeCanvas(){
	...
	*/
	...
	var posX = 25, posY = anim.height/2;
	...
	setInterval(function(){
		...
		if( posX + circleRadius >= anim.width ){directionX = false;}

		//Checks the collision from the bottom wall
		if(posY + circleRadius >= anim.height){directionY = false;}

		//Checks the collision from the top wall
		if(posY <= circleRadius){directionY = true;}

		//Checks the collision from the left wall
		if( posX <= circleRadius){directionX = true;}

		if( directionX == false){ posX -= speedX;}
		else if(directionX == true){posX += speedX; }


		if( directionY == false){ posY -= speedY;}
		else if(directionY == true){posY += speedY;}

		document.getElementById("info").innerHTML = "posX:" + posX + " posY:" + posY + " directionY:" + directionY + " directionX:" + directionX;
		animationSpeed++;
